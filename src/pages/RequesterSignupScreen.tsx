  ...
  return (
    <div className="min-h-screen bg-background max-w-lg mx-auto pb-12">
      <div className="px-6 pt-12 pb-6">
        ...
        <p className="text-xs font-bold uppercase tracking-widest text-muted-foreground mb-1">Patient / Attender</p>
            ...
            <div key={label} className="flex items-center">
              <div className="flex flex-col items-center">
                <div className={`flex items-center justify-center w-8 h-8 rounded-full text-xs font-bold transition-all ${
                  ...
                  : i === stepIdx ? "bg-primary text-primary-foreground shadow-[0_0_0_4px_hsl(var(--primary)/0.2)]"
                  : "bg-muted text-muted-foreground"
                }`}>
                  {i < stepIdx ? "✓" : i + 1}
                </div>
                <span className={`text-[10px] font-semibold mt-1 ${i === stepIdx ? "text-primary" : "text-muted-foreground"}`}>{label}</span>
              </div>
      ...
      <AnimatePresence mode="wait">
        ...
        {step === "verify_phone" && (
          <motion.div key="verify_phone"
            ...
            transition={{ duration: 0.22 }} className="px-6 space-y-4">
            ...
            <div className="p-4 rounded-2xl bg-primary/5 border border-primary/20 flex gap-3">
              ...
              <div>
                ...
                <p className="text-sm text-muted-foreground mt-0.5">Helps donors contact you in emergencies.</p>
              </div>
            </div>
                ...
                {!otpSent ? (
                  <>
                    ...
                    <motion.button whileTap={{ scale: 0.97 }} onClick={handleSendOtp}
                      ...
                      {loading ? <span className="flex items-center justify-center gap-2"><span className="w-4 h-4 border-2 border-primary-foreground/30 border-t-primary-foreground rounded-full animate-spin" />Sending…</span> : "Send OTP →"}
                    </motion.button>
                  </>
                ) : (
                  <>
                    ...
                    <button onClick={() => { setOtpSent(false); setOtp(""); setPendingOtp(""); }}
                      className="w-full text-sm text-muted-foreground text-center py-1">← Change phone number</button>
                  </>
                )}
              </>
            ) : (
            ...
            {!phoneVerified && (
              <button onClick={() => setStep("details")} className="w-full text-sm text-muted-foreground text-center py-2">
                Skip for now →
              </button>
            )}
          </motion.div>
        ...
        {step === "details" && (
          <motion.form key="details"
            ...
            onSubmit={handleCreateAccount} className="px-6 space-y-3">
            ...
            {verifiedPhone && (
              <div className="flex items-center gap-2 px-4 py-2 rounded-xl bg-green-500/10 border border-green-500/30 text-green-600 text-sm font-semibold">
                <CheckCircle className="w-4 h-4" /> Phone verified: {verifiedPhone}
              </div>
            )}
            ...
            <motion.button whileTap={{ scale: 0.97 }} type="submit" disabled={loading}
              ...
              {loading ? <span className="flex items-center justify-center gap-2"><span className="w-4 h-4 border-2 border-primary-foreground/30 border-t-primary-foreground rounded-full animate-spin" />Creating…</span> : "Create Account →"}
            </motion.button>

            <div className="relative my-1">
              <div className="absolute inset-0 flex items-center"><div className="w-full border-t border-border" /></div>
              <div className="relative flex justify-center"><span className="bg-background px-4 text-[11px] font-bold uppercase tracking-widest text-muted-foreground/50">or</span></div>
            </div>
        ...
        {step === "location" && (
          <motion.div key="location"
            ...
            <div className="p-4 rounded-2xl bg-primary/5 border border-primary/20 flex gap-3">
              ...
              <div>
                ...
                <p className="text-sm text-muted-foreground mt-0.5">
                  Helps donors near you respond faster. Updates automatically on each app open.
                </p>
              </div>
            </div>
            ...
            <motion.button whileTap={{ scale: 0.97 }} onClick={handleGetLocation} disabled={loading}
              className="w-full h-14 bg-primary text-primary-foreground rounded-2xl font-bold text-[15px] shadow-button disabled:opacity-60">
              {loading ? <span className="flex items-center justify-center gap-2"><span className="w-4 h-4 border-2 border-primary-foreground/30 border-t-primary-foreground rounded-full animate-spin" />Saving…</span> : "📍 Enable Location Access"}
            </motion.button>
